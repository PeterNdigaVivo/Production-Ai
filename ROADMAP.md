...
**Reporting scope — what actually works today.** AWAY is now emitted for absences **shorter than the query window** and where the seat has had at least one confirmed state after engine start. Three known holes remain, and productivity should NOT be described as reliable until all three are closed:

1. **Long absences across window edges.** Their single AWAY event falls outside the queried window and the intervals CTE only sees `ts >= window_start`. The longer a seat is empty, the more likely its lone AWAY event has already fallen out of any short recent window — the true-north case (a three-hour empty seat over a 10-minute recent window) is exactly the shape this misses.
2. **Pipeline outages inside the window.** During an outage no `worker_state_changed` events are written, and `intervals.py` closes each interval with `LEAD(ts)` — so the entire gap gets attributed to whichever state was last recorded. A 30-minute ffmpeg drop on a seat that was WORKING reads as 30 minutes of WORKING. The outage guard in the activity engine protects the FSM's debounce vote; it does nothing to the duration query.
3. **Chronically empty seat.** Observed on the tower on the first live 30-minute window after deploy: three seats that were empty for the whole window produced ZERO rows — not one AWAY, nothing. The FSM starts at UNKNOWN, the handler emits only on `new_state != prev_state`, so a seat that has been empty since engine start confirms AWAY once and is silent forever. In the CTE this is indistinguishable from a workstation that does not exist. This is a distinct shape from (1) — (1) is "the event exists but before `window_start`"; (3) is "no event exists anywhere for this workstation, ever". The worst case on the factory floor (a seat abandoned all shift) is currently invisible.

All three are the same failure family: the interval math has no way to say what an *absence of events* means, and none of them is fixable in the activity engine. **Fold all three into the next task**, framed as *"what does a gap in worker_events mean?"* rather than only *"states open before window_start"*. When the fix lands, do it as the CTE modification, not the Deferred #3 snapshot-row idea: snapshot rows touch every downstream consumer and break the transitions-only semantics lesson #2 rests on; the CTE fix is contained in one function.

**Design proposal required before any implementation.** The proposal must show explicitly how the CTE distinguishes:

- **(a)** seat with a state confirmed before `window_start` and open at it;
- **(b)** seat with no rows at all (in or before the window);
- **(c)** a pipeline outage inside the window.

And it must state plainly whether **(b) is solvable in the CTE alone or needs the roster as an input**. If it needs the roster, say so — that is a real design constraint. Looking back further in `worker_events` cannot help with (b) because there is nothing further back; the set of workstations that ought to exist has to come from somewhere else (the `workstations` table, or a periodic-roster snapshot, or something new). Flag that trade-off in the proposal, do not paper over it.

The obvious synthesise-a-gap-marker move IS Deferred #3 in disguise — call it out and reject it explicitly in the proposal.
